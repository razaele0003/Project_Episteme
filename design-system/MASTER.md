...
Applied direction: a calm personal learning workspace, forest-green progress panel, warm-white surfaces, restrained sage accents, DM Sans bundled locally, consistent Lucide icons, visible keyboard focus, semantic progress element, reduced motion and mobile layout. The forest palette and DM Sans are project-specific editorial choices, not claimed database recommendations. Starter values come only from the backend. Never use invented completion percentages, streaks or activity.
