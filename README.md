...
Phase 0 links learners directly to the shared Episteme Coach. The coach is hint-first, uses this repository as its curriculum reference, inspects only a learner-authorized repository, separates source review from executed tests, and never marks repository evidence as mastery. ChatGPT's GitHub connection is separate from Episteme's connection and remains read-only.
